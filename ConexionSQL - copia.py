...
class Conexion:
    ...
    def crear_conexion_MySQL(self, user, password, host, database):
        self.conn = mysql.connector.connect(user=user, password=password, host=host, database=database)

    def crear_conexion_SQLite(self):
        self.conn = sqlite3.connect("../Datos_Olimpiadas/ediciones_olimpicas.db")

    def crear_BBDD(self, mySql):
            ...
            with open(path) as csvFile:
                ...
                for athlete in reader:
                    ...
                    if athlete[0] != mismo_athleta:
                        ...
                        if athlete[5] == 'NA':
                            peso = 0
                        self.__insert_into_deportista__(athlete[0], athlete[1], athlete[2], altura, peso, mySql)
    ...
    def listar_deporistas_participan_diferentes_deportes(self, user, password, host, database):
        ...
        if bbddd.__eq__("mysql"):
            mySQL = True
            self.crear_conexion_MySQL(user, password, host, database)
        else:
            mySQL = False
            self.crear_conexion_SQLite()
    ...
    def borrar_BBDD(self, mySql, user="", password="", host=""):
        print("Borrar contenido..")
        if(mySql):
